...
PSIMD_INTRINSIC psimd_f32 fp16_ieee_to_fp32_psimd(psimd_u16 half) {
	...
	const psimd_u32 shr3_nonsign = (word + word) >> 4;

	const psimd_u32 exp_offset = psimd_splat_u32(UINT32_C(0x70000000));
	const psimd_f32 exp_scale = psimd_splat_f32(0x1.0p-112f);
	const psimd_f32 norm_nonsign = psimd_mul_f32(to_float(shr3_nonsign + exp_offset), exp_scale);

	const psimd_u16 magic_mask = psimd_splat_u16(UINT16_C(0x3E80));
	const psimd_f32 magic_bias = psimd_splat_f32(0.25f);
	const psimd_f32 denorm_nonsign = psimd_sub_f32(to_float(Vec4i(psimd_interleave_lo_u16(half + half, magic_mask))), magic_bias);
...
PSIMD_INTRINSIC psimd_f32x2 fp16_ieee_to_fp32x2_psimd(psimd_u16 half) {
	...
	const psimd_u32 shr3_nonsign_lo = (word_lo + word_lo) >> 4;
	const psimd_u32 shr3_nonsign_hi = (word_hi + word_hi) >> 4;

	const psimd_u32 exp_offset = psimd_splat_u32(UINT32_C(0x70000000));
	const psimd_f32 exp_scale = psimd_splat_f32(0x1.0p-112f);
	const psimd_f32 norm_nonsign_lo = psimd_mul_f32(to_float(shr3_nonsign_lo + exp_offset), exp_scale);
	const psimd_f32 norm_nonsign_hi = psimd_mul_f32(to_float(shr3_nonsign_hi + exp_offset), exp_scale);

	const psimd_u16 magic_mask = psimd_splat_u16(UINT16_C(0x3E80));
	const psimd_u16 shl1_half = half + half;
	const psimd_f32 magic_bias = psimd_splat_f32(0.25f);
	const psimd_f32 denorm_nonsign_lo = psimd_sub_f32(to_float(Vec4i(psimd_interleave_lo_u16(shl1_half, magic_mask))), magic_bias);
	const psimd_f32 denorm_nonsign_hi = psimd_sub_f32(to_float(Vec4i(psimd_interleave_hi_u16(shl1_half, magic_mask))), magic_bias);
	...
	psimd_f32x2 result;
	result.lo = to_float(sign_lo | truncate_to_int(psimd_blend_f32(denorm_mask_lo, denorm_nonsign_lo, norm_nonsign_lo)));
	result.hi = to_float(sign_hi | truncate_to_int(psimd_blend_f32(denorm_mask_hi, denorm_nonsign_hi, norm_nonsign_hi)));
	return result;
}
...
PSIMD_INTRINSIC psimd_f32 fp16_alt_to_fp32_psimd(psimd_u16 half) {
	...
	const psimd_f32 nonsign = to_float(shr3_nonsign + exp_offset);
	const psimd_f32 denorm_bias = psimd_splat_f32(0x1.0p-14f);
	return to_float(sign | truncate_to_int(psimd_sub_f32(psimd_add_f32(nonsign, nonsign), psimd_max_f32(nonsign, denorm_bias))));
#endif
}
...
PSIMD_INTRINSIC psimd_f32x2 fp16_alt_to_fp32x2_psimd(psimd_u16 half) {
	...
	const psimd_s32 exp1_offset = psimd_splat_s32(INT32_C(0x00800000));
	const psimd_f32 two_nonsign_lo = to_float(nonsign_bits_lo + exp1_offset);
	const psimd_f32 two_nonsign_hi = to_float(nonsign_bits_hi + exp1_offset);
	const psimd_s32 exp113_offset = exp1_offset | exp112_offset;
	psimd_f32x2 result;
	result.lo = to_float(sign_lo | truncate_to_int(psimd_sub_f32(two_nonsign_lo, to_float(psimd_max_s32(nonsign_bits_lo, exp113_offset)))));
	result.hi = to_float(sign_hi | truncate_to_int(psimd_sub_f32(two_nonsign_hi, to_float(psimd_max_s32(nonsign_bits_hi, exp113_offset)))));
	return result;
#else
	const psimd_u32 exp_offset = psimd_splat_u32(UINT32_C(0x38000000));
