	...
	const size_t input_channel)
{
	...
	for (size_t y = 0ull; y < input_size.height; y++) 
		for (size_t x = 0ull; x < input_size.width; x++) 
		{
			double v = 0.0;
			for (size_t output_channel = 0ull; output_channel < output_channels; output_channel++) 
				for (size_t i = 0ull; i < kernel_size.height; i++) 
				{
					const size_t s = y - i + input_padding.top;
					if (s < output_size.height) 
						for (size_t j = 0ull; j < kernel_size.width; j++) 
						{
