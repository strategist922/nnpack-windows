	...
	const size_t input_channel)
{
	...
	for (size_t y = 0ull; y < kernel_size.height; y++) 
		for (size_t x = 0ull; x < kernel_size.width; x++) 
		{
			double grad_kernel_yx = 0.0;
			for (size_t sample = 0ull; sample < batch_size; sample++) 
				for (size_t i = 0ull; i < output_size.height; i++) 
				{
					const size_t s = y + i - input_padding.top;
					if (s < input_size.height) 
						for (size_t j = 0ull; j < output_size.width; j++) 
						{
	...
	float* grad_kernel)
{
	...
	pthreadpool_compute_2d((pthreadpool_function_2d_t)compute_convolution_kernel_gradient,
