...
__m256 _mm256_exp_ps(__m256 x) {
	...
	const __m256 log2e = _mm256_set1_ps(std::strtof("0x1.715476p+3f", NULL));
	const __m256 minus_ln2_hi = _mm256_set1_ps(std::strtof("-0x1.62E430p-4f", NULL) );
	const __m256 minus_ln2_lo = _mm256_set1_ps(std::strtof("0x1.05C610p-32f", NULL) );
	const __m256 plus_inf = _mm256_set1_ps(std::numeric_limits<float>::infinity());

	const __m256 c2 = _mm256_set1_ps(std::strtof("0x1.00088Ap-1f", NULL));
	const __m256 c3 = _mm256_set1_ps(std::strtof("0x1.555A86p-3f", NULL));
	const __m256 table = _mm256_set_ps(std::strtof("0x1.D5818Ep+0f", NULL), std::strtof("0x1.AE89FAp+0f", NULL), std::strtof("0x1.8ACE54p+0f", NULL), std::strtof("0x1.6A09E6p+0f", NULL), std::strtof("0x1.4BFDAEp+0f", NULL), std::strtof("0x1.306FE0p+0f", NULL), std::strtof("0x1.172B84p+0f", NULL), std::strtof("0x1.000000p+0f", NULL));
